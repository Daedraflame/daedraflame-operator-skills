...
description: Discord and community conversion using 4 Rs, rituals, activation, referrals, regulars, owned audience, and retention loops.
version: 1.1.0
...
Retrieval keywords: Discord, community conversion, 4 Rs, regulars, rituals, activation, owned audience, referrals, retention
...
Move casual viewers into an owned community where they activate, return, refer, and support.

## When to use

Use for Discord inactivity, community rituals, regulars, viewer-to-member conversion, event design, referrals, and owned audience retention.

## When NOT to use

Do not use for short-form top-of-funnel, title packaging, stream pacing, or monetisation unless community is the conversion layer.

## Inputs to look for

Discord joins, activation rate, active members, events, rituals, onboarding, channels, referral prompts, regulars, stream CTA, community proof.
...
- Use the 4 Rs: Retain members, Review moments and wins, Refer through community proof, Resell events or supporter offers.
- A join promise must be specific; nobody joins a vague server for long.
- Activation beats server size.
- Rituals reduce effort/sacrifice and make return behaviour automatic.
- Community proof should feed acquisition and monetisation.
...
Track: Discord joins per stream, activation rate, weekly active members, event attendance, referral posts, repeat chatters, supporter conversion, member-generated proof.
...
- My Discord is dead.
- Apply the 4 Rs to my community.
- How do I turn viewers into regulars?

## Example ideal response style

```text # Bottleneck Your bottleneck is activation, not server size. People join without a job, ritual, or proof that returning gives them a payoff.  # Economic truth The system is paid by proof and conversion, not effort. One Thing gets fixed first; everything else is secondary until the scoreboard moves.  # What to stop doing Stop spreading attention across five comfortable tasks. Pick the constraint that blocks the next dollar, viewer, lead, or proof asset.  # Next measurable action Create one join promise, one first action, one weekly ritual, and track activation rate plus weekly active members.  # Scoreboard Track the one conversion metric that proves whether the bottleneck moved this week.  # Need next Send the current numbers, the last 7 days of shipped actions, and the one constraint you believe is blocking growth. ```

## Failure modes to avoid

- Adding channels instead of rituals.
- Measuring members instead of active members.
- No onboarding action.
- No referral loop.
- Treating Discord as storage instead of a conversion system.
